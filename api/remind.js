...
export default async function handler(req, res) {
  ...
  const zoomBlock = session.zoom_link
    ? `<div style="text-align:center;margin-bottom:24px"><a href="${session.zoom_link}" style="display:inline-block;padding:14px 36px;background:#B91C3C;color:#fff;border-radius:8px;font-size:16px;font-weight:600;text-decoration:none">Join Zoom Session</a></div><p style="font-size:13px;color:#7A7A7A;text-align:center;margin-bottom:24px">Zoom link: <a href="${session.zoom_link}" style="color:#B91C3C">${session.zoom_link}</a></p>`
    : '<p style="font-size:14px;color:#4A4A4A;line-height:1.6;margin-bottom:24px">The Zoom link will be shared separately before the session.</p>'
...
<div style="max-width:600px;margin:0 auto;padding:32px 16px">
  ...
  <div style="background:#fff;padding:32px 28px;border:1px solid #E0DCD5;border-top:none">
    ...
    ${zoomBlock}
    ${session.description ? `<p style="font-size:14px;color:#4A4A4A;line-height:1.6;margin-bottom:24px">${session.description}</p>` : ''}
    <p style="font-size:14px;color:#4A4A4A;line-height:1.6">After the session, please log in to the <a href="${siteUrl}" style="color:#B91C3C;font-weight:600">CHeSS Member Dashboard</a> to complete your evaluation and claim CME credit.</p>
    <p style="font-size:14px;color:#4A4A4A;line-height:1.6;margin-top:24px">Best regards,<br><strong>${signer}</strong></p>
  </div>
